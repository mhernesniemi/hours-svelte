<script lang="ts">
  ...
  import { ArrowLeft, Save, Loader2, AlertCircle, Trash2, Search } from "@lucide/svelte";
  import { format, set, startOfDay, parseISO } from "date-fns";

  // Get entry ID from URL
  const entryId = parseInt(page.params.id ?? "0");
  ...
  $effect(() => {
    if (!isLoaded) {
      phasesPromise.then((phases) => {
        // We need to find this entry - for now, just load today's entries
        // In a real app, you'd have a getEntryById remote function
        loadEntry(phases);
      });
    }
  });

  async function loadEntry(phases: Awaited<typeof phasesPromise>) {
    // This is a simplified approach - ideally have a dedicated getEntry function
    // For now, we'll rely on the user navigating from the main page
    // The data would typically come from page data or a store
    isLoaded = true;

    // Set defaults - in real app, load from actual entry data
    ...
    <Card>
      <CardHeader>
        <div class="flex items-center justify-between">
          <CardTitle>Edit Hour Entry</CardTitle>

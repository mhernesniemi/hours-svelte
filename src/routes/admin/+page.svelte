<script lang="ts">
	...
	async function handleSync(type: 'all' | 'customers' | 'projects' | 'phases' | 'worktypes' | 'users') {
		isSyncing = type;
		syncResult = null;
	...
	{:else}
		<div class="mx-auto max-w-5xl p-4">
					...
					<div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
						...
						>
							...
							<div class="text-left">
								...
								<div class="text-muted-foreground text-xs">Import all data</div>
							</div>
						</Button>
						...
						>
							...
							<div class="text-left">
								...
								<div class="text-muted-foreground text-xs">Import customers</div>
							</div>
						</Button>
						...
						>
							...
							<div class="text-left">
								...
								<div class="text-muted-foreground text-xs">Import projects/cases</div>
							</div>
						</Button>
						...
						>
							...
							<div class="text-left">
								...
								<div class="text-muted-foreground text-xs">Import project phases</div>
							</div>
						</Button>
						...
						>
							...
							<div class="text-left">
								...
								<div class="text-muted-foreground text-xs">Import work types</div>
							</div>
						</Button>
						...
						>
							...
							<div class="text-left">
								...
								<div class="text-muted-foreground text-xs">Import users</div>
							</div>
						</Button>
					</div>
				</CardContent>
			...
			<Card class="mb-6">
				...
				<CardContent>
					{#await usersPromise}
						<div class="text-muted-foreground py-8 text-center">Loading users...</div>
					{:then users}
						{#if users.length === 0}
								...
								{#each users as u}
									<div class="flex items-center justify-between py-3 first:pt-0 last:pb-0">
										<div>
											...
											<div class="text-muted-foreground text-sm">{u.email}</div>
										</div>
						...
						{/if}
					{:catch error}
						<div class="text-destructive py-8 text-center">Failed to load users</div>
					{/await}
				</CardContent>
			</Card>
			...
			<Card>
				...
				<CardContent>
					{#await syncLogsPromise}
						<div class="text-muted-foreground py-8 text-center">Loading logs...</div>
					{:then logs}
						{#if logs.length === 0}
							<div class="text-muted-foreground py-8 text-center">No sync logs found.</div>
						{:else}
							<div class="divide-border divide-y">
								{#each logs.slice(0, 10) as log}
									<div class="flex items-center justify-between py-3 first:pt-0 last:pb-0">
										...
										<div class="text-right">
											...
											{/if}
											{#if log.error}
												<div class="text-destructive max-w-xs truncate text-xs">{log.error}</div>
											{/if}
										</div>
									</div>
								{/each}
						...
						{/if}
					{:catch error}
						<div class="text-destructive py-8 text-center">Failed to load logs</div>
					{/await}
				</CardContent>
			</Card>
		</div>
